...
function SetLevel takes nothing returns nothing
    ...
    if GetStoredInteger(udg_GC[99], "", "board_number") > 0 then
        set y = x + 1000 * (0.01 * GetStoredInteger(udg_GC[99], "", "winrate") * (0.01 * GetStoredInteger(udg_GC[99], "", "board_number") + 1)) * (GetStoredInteger(udg_GC[99], "", "udg_MVP") * 0.5 / GetStoredInteger(udg_GC[99], "", "board_number") + 1) * (GetStoredInteger(udg_GC[99], "", "mighty") * 0.5 / GetStoredInteger(udg_GC[99], "", "board_number") + 1)
    endif
...
function ReAddDummy takes nothing returns nothing
    ...
    call SetUnitX(u, -5344.0)
    call SetUnitY(u, -3968.0)
    call ReleaseTimer(t)
...
function ReleaseDummy takes unit u returns nothing
    ...
    if IsUnitInGroup(u, udg_DummyCastersAll[GetPlayerId(GetOwningPlayer(u))]) then
        ...
        call TimerStart(t, 5.0, false, function ReAddDummy)
    else
...
function ReleaseSpecialDummy takes unit u returns nothing
    ...
    if IsUnitInGroup(u, udg_SpecialDummyCastersAll[GetPlayerId(GetOwningPlayer(u))]) then
        set t = CreateTimer()
        call SaveUnitHandle(udg_ht, GetHandleId(t), 0, u)
        call TimerStart(t, 5.0, false, function ReAddSpecialDummy)
    else
        call ShowUnit(u, false)
        call SetUnitExploded(u, true)
        call KillUnit(u)
...
function InitCasters takes nothing returns nothing
    ...
    loop
    exitwhen i > 11
        set udg_Caster[i] = CreateUnit(Player(i), 'e02U', -5344.0, -3968.0, 0.0)
        set i = i + 1
    endloop
endfunction
...
function TimedLightning takes lightning e, real fadetime returns nothing
    ...
    call TimerStart(t, 0.05, true, function TimedLightningClear)
    set t = null
endfunction
...
function ClearUnitFromTarget takes nothing returns nothing
    ...
    if timeleft > 0 then
        ...
        if GetWidgetLife(target) < 0.405 or IsUnitType(target, UNIT_TYPE_DEAD) and killondeath then
...
function AttachUnitToTarget takes unit u, unit target, real duration, boolean killondeath, boolean killonexpire, boolean playdeathanim returns nothing
    local timer t = CreateTimer()
    local integer task = GetHandleId(t)
    call SaveReal(udg_ht, task, 0, duration - 0.02)
    call SaveUnitHandle(udg_ht, task, 0, u)
    ...
    call SetUnitX(u, GetUnitX(target))
    call SetUnitY(u, GetUnitY(target))
    call TimerStart(t, 0.02, true, function ClearUnitFromTarget)
    set t = null
endfunction
...
function Knockback_UnitFilter takes nothing returns boolean
    ...
    if u != LoadUnitHandle(udg_ht, task, 0) and u != LoadUnitHandle(udg_ht, task, 1) then
        if GetWidgetLife(u) > 0.405 and GetUnitAbilityLevel(u, 'Aloc') == 0 and not IsUnitType(u, UNIT_TYPE_STRUCTURE) and not IsUnitType(u, UNIT_TYPE_ANCIENT) and not IsUnitType(u, UNIT_TYPE_DEAD) then
            set u = null
            set t = null
            return true
        endif
...
function Knockback_UnitCheck takes nothing returns boolean
    ...
    local timer t = GetExpiredTimer()
    local integer task = GetHandleId(t)
    if GetWidgetLife(u) > 0.405 and u != LoadUnitHandle(udg_ht, task, 0) and u != LoadUnitHandle(udg_ht, task, 1) and not IsUnitType(u, UNIT_TYPE_DEAD) then
...
function Knockback_Loop takes nothing returns nothing
    ...
    if knockothers then
        ...
        call GroupEnumUnitsInRange(g, cx, cy, 75.0, b1)
        loop
        ...
        exitwhen v == null
            ...
            if v != caster and not IsUnitInGroup(v, udg_KnockedBack) and (IsUnitAlly(v, GetOwningPlayer(caster)) and knockfriendly) or IsUnitEnemy(v, GetOwningPlayer(caster)) then
                ...
                call SaveReal(udg_ht, i, 2, 0.75 * (distance - SquareRoot((cx - ox) * (cx - ox) + (cy - oy) * (cy - oy))))
                call SaveReal(udg_ht, i, 3, 0.5 * speed)
        ...
        call GroupEnumUnitsInRange(g, cx + 32 * 75.0 * cos, cy + 32 * 75.0 * sin, 75.0, b2)
        call GroupClear(g)
    endif
    ...
    if IsUnitInRangeXY(target, ox, oy, distance) then
        ...
        if IsTerrainPathable(cx, cy, PATHING_TYPE_WALKABILITY) == false and continue then
            ...
            call SetRect(r, cx + 32 * 84.0 * cos - 84.0, cy + 32 * 84.0 * sin - 84.0, cx + 32 * 84.0 * cos + 84.0, cy + 32 * 84.0 * sin + 84.0)
            call SaveRectHandle(udg_ht, task, 16, r)
        else
            set e = LoadEffectHandle(udg_ht, task, 10)
            if LoadBoolean(udg_ht, task, 21) then
...
function Knockback takes unit caster, unit target, real distance, real speed, real angle, string e, boolean realistic, boolean knockothers, boolean knockfriendly, boolean stopontrees returns nothing
    ...
    local group g = CreateGroup()
    local rect r = Rect(ox - 84.0, oy - 84.0, ox + 84.0, oy + 84.0)
    ...
    call SaveReal(udg_ht, task, 8, 0.03125 * Cos(angle))
    call SaveReal(udg_ht, task, 9, 0.03125 * Sin(angle))
    ...
    if GetUnitAbilityLevel(target, 'A17X') == 0 and GetUnitAbilityLevel(target, 'A0PF') == 0 and GetUnitAbilityLevel(target, 'A0AN') == 0 then
        ...
        call SaveBoolean(udg_ht, task, 21, false)
    endif
    call GroupAddUnit(udg_KnockedBack, target)
    call TimerStart(t, 0.03125, true, function Knockback_Loop)
    set t = null
    set g = null
    set b = null
    set b1 = null
...
function GroupEnumUnitsInRectangle takes group g, real ox, real oy, real ux, real uy, real rx, real ry, boolexpr f returns nothing
    local real mx = 0.5 * (ux + rx)
    local real my = 0.5 * (uy + ry)
...
function ProjectileMoveToTarget takes nothing returns nothing
    ...
    if IsUnitInRange(projectile, target, d) or GetWidgetLife(target) < 0.405 then
        set e = LoadEffectHandle(udg_ht, task, 3)
        set source = LoadUnitHandle(udg_ht, task, 0)
        call DestroyEffect(e)
        call SetUnitScale(projectile, 1.0, 1.0, 1.0)
...
function LaunchProjectileToUnit takes string model, real scale, unit source, real speed, unit target, string func returns nothing
    ...
    call TimerStart(t, 0.03125, true, function ProjectileMoveToTarget)
    set u = null
    set t = null
    set e = null
endfunction
...
function ProjectileMoveToPoint takes nothing returns nothing
    ...
    exitwhen v == null
        ...
        if v != source and GetWidgetLife(v) > 0.405 and not IsUnitInGroup(v, controlgrp) and IsUnitType(v, UNIT_TYPE_STRUCTURE) == false and GetUnitAbilityLevel(v, 'Avul') == 0 then
...
function LaunchProjectileFromUnitToPoint takes string model, real scale, unit source, real tx, real ty, real speed, real collisionsize, string onhitfunc, string onendfunc, boolean stoponally, boolean stoponenemy, boolean stoponhero returns nothing
    ...
    call TimerStart(t, 0.03125, true, function ProjectileMoveToPoint)
    set t = null
    set u = null
    set e = null
    set g = null
...
function IsUnitDead takes unit u returns boolean
    return GetWidgetLife(u) < 0.405 or IsUnitType(u, UNIT_TYPE_DEAD) or GetUnitTypeId(u) == 0
endfunction
...
function TSU_UnitAddItem_Main takes nothing returns nothing
    ...
    local integer i = LoadInteger(udg_ht, task, 1)
    if GetWidgetLife(u) >= 0.405 or i <= 0 then
...
function CameraChatActions takes nothing returns nothing
    ...
    if bj_isUnitGroupDeadResult and GetEventPlayerChatString() == "-fol" then
        ...
        call DisplayTimedTextToPlayer(GetTriggerPlayer(), 0.0, 0.0, 2.0, "Entered camera following mode, press Esc to temporarily stop the effect for 10 seconds or enter '-foloff' to cancel")
    ...
    elseif GetEventPlayerChatString() == "-foloff" then
        call DisplayTimedTextToPlayer(GetTriggerPlayer(), 0.0, 0.0, 2.0, "Camera following mode turned off")
...
function CameraEscape takes nothing returns nothing
    ...
    if IsUnitInGroup(udg_PlayerHeroes[i], udg_CameraUnits) then
        call DisplayTimedTextToPlayer(GetTriggerPlayer(), 0.0, 0.0, 2.0, "Camera following mode temporarily suspended for 10 seconds")
        call CameraRemove(udg_PlayerHeroes[i])
    else
        return
    endif
    call TriggerSleepAction(10.0)
    if GetWidgetLife(udg_PlayerHeroes[i]) > 0.405 then
        call DisplayTimedTextToPlayer(GetTriggerPlayer(), 0.0, 0.0, 2.0, "Camera following mode turned on")
...
function GCU_Enum takes nothing returns nothing
    ...
    local real d = (dx * dx + dy * dy) / 10000.0
