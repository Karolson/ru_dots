function Trig_Jump16_Conditions takes nothing returns boolean
...
function Trig_Jump16_Actions takes nothing returns nothing
    local unit u = GetTriggerUnit()
    local location a = GetRectCenter(gg_rct_Jump16top)
    local location b = GetRectCenter(gg_rct_Jump16bot)
    local real ang = AngleBetweenPoints(a, b)
    local real dst = DistanceBetweenPoints(a, b)
    local effect e
    call DisableTrigger(gg_trg_Jump16)
    ...
    call EnableTrigger(gg_trg_Jump16)
...
function InitTrig_Jump16 takes nothing returns nothing
    set gg_trg_Jump16 = CreateTrigger()
    call YDWETriggerRegisterEnterRectSimpleNull(gg_trg_Jump16, gg_rct_Jump16top)
    call TriggerAddCondition(gg_trg_Jump16, Condition(function Trig_Jump16_Conditions))
    call TriggerAddAction(gg_trg_Jump16, function Trig_Jump16_Actions)
endfunction