function Trig_YoumuN02_Conditions takes nothing returns boolean
    if GetUnitAbilityLevel(GetEventDamageSource(), 'A1GH') == 0 then
        return false
...
function Trig_YoumuN02_Fade takes nothing returns nothing
...
function Trig_YoumuN02_Actions takes nothing returns nothing
    ...
    if IsUnitType(target, UNIT_TYPE_STRUCTURE) == false and not IsUnitIllusion(caster) then
        set b = false
        set level = GetUnitAbilityLevel(caster, 'A1GH')
        ...
        if b then
            ...
            call TimerStart(t, 1.0, true, function Trig_YoumuN02_Fade)
        endif
...
function InitTrig_YoumuN02 takes nothing returns nothing
endfunction