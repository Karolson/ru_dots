function Trig_YoumuN01_Main takes nothing returns nothing
    ...
    if GetWidgetLife(caster) >= 0.405 and i < 2 then
        set damage = ABCIAllAtk(caster, 0 - 5 + level * 15, 0.6)
        set g = CreateGroup()
...
function Trig_YoumuN01_Conditions takes nothing returns boolean
    return GetSpellAbilityId() == 'A1GG'
endfunction

function Trig_YoumuN01_Actions takes nothing returns nothing
    ...
    local timer t
    local integer task
    if GetUnitAbilityLevel(caster, 'A1GJ') > 1 then
        call AbilityCoolDownResetion(caster, GetSpellAbilityId(), 0.1)
        call SetUnitAbilityLevel(caster, 'A1GJ', GetUnitAbilityLevel(caster, 'A1GJ') - 1)
    else
        call AbilityCoolDownResetion(caster, GetSpellAbilityId(), 10.5 - 1.5 * level)
    endif
    ...
    call TimerStart(t, 0.2, true, function Trig_YoumuN01_Main)
    set caster = null
    set u = null
    set t = null
endfunction

function InitTrig_YoumuN01 takes nothing returns nothing
