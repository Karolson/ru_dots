...
function RestrictTarget takes unit source, unit target, real duration returns nothing
    ...
    local timer t
    if GetUnitAbilityLevel(target, 'B097') > 0 then
        return
    endif
    ...
    if not IsUnitIllusion(target) and IsUnitType(target, UNIT_TYPE_HERO) and udg_BlinkEnableUnit[GetPlayerId(GetOwningPlayer(target))] != null and GetUnitAbilityLevel(target, 'B097') == 0 then
        set t = CreateTimer()
