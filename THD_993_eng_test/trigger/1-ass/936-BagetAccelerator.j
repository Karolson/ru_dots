...
function Trig_BagetAccelerator_Move takes nothing returns nothing
    ...
    endloop
    call DestroyGroup(g)
    if IsUnitCCImmune(target) and IsUnitEnemy(target, p) then
        call ReleaseTimer(LoadTimerHandle(udg_ht, task, 5))
        set tend = CreateTimer()
        call SaveTimerHandle(udg_ht, GetHandleId(tend), 0, t)
        call TimerStart(tend, 0.0, false, function Trig_BagetAccelerator_Move_End)
        set g = null
        set tend = null
        return
    endif
...
function Trig_BagetAccelerator_Actions takes nothing returns nothing
    ...
    call SaveTimerHandle(udg_ht, task, 5, tend)
