    ...
    if SquareRoot((ox - tx) * (ox - tx) + (oy - ty) * (oy - ty)) < mspeed + 4.0 then
        ...
        if sktype == 'A0MT' then
            if IsUnitAlly(caster, GetOwningPlayer(target)) == false then
                ...
                call UnitMagicDamageTarget(caster, target, (30 + 35 * sklevel + 1.7 * GetHeroInt(caster, true)) * jumpdeplz * kmswitchincre, 1)
            endif
            if kmswitch then
                ...
                exitwhen v == null
                    ...
                    call UnitMagicDamageTarget(caster, v, (30 + 35 * sklevel + 1.7 * GetHeroInt(caster, true)) * (0.25 * (1 + kmlevel)) * jumpdeplz * kmswitchincre * 0.75, 5)
                endloop
                call DestroyGroup(g)
            endif
        endif
