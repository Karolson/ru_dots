function Trig_AI_Koishi_Attacked_Conditions takes nothing returns boolean
    if IsPlayerInForce(GetOwningPlayer(GetTriggerUnit()), udg_AI_Players) == false then
        return false
    endif
    return GetUnitTypeId(GetTriggerUnit()) == 'E010'
endfunction

function Trig_AI_Koishi_Attacked_Actions takes nothing returns nothing
    ...
    elseif IsUnitType(attacker, UNIT_TYPE_HERO) then
        ...
        call IssueImmediateOrder(h, "stomp")
...
function InitTrig_AI_Koishi_Attacked takes nothing returns nothing
endfunction